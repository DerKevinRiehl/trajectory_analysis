...
def reconstruct_trajectories_cvxopt(trajectory_df: pd.DataFrame, accel_max_spl: Optional[PPoly] = None, decel_min_spl: Optional[PPoly] = None, 
    ...
    -----------
        ...
        accel_max_spl: Optional[PPoly] = None
            Cubic spline definig the acceleration capacity versus speed curve for a model vehicle
        decel_min_spl: Optional[PPoly] = None
            Cubic spline definig the acceleration capacity versus speed curve for a model vehicle
        vehicle_info_df: Optional[pd.DataFrame] = None
            contains the vehicle information for the current experiment, specifically the MFC_CarID for importing the relevant accel_max_spl and decel_min_spl
        ...
        weight_speed_noise: Optional[float] = 1.0
    ...
    """
    if vehicle_info_df is None:
        assert(accel_max_spl is not None and decel_min_spl is not None)
    ...
    while len(remaining_vehicles) > 0:
        ...
        # Get the relevant accel_max_spl and decel_min_spl if required based on MFC_CarID
        if vehicle_info_df is not None:
        ...
        for k in range(0, len(speed), step):
            ...
            if relax_accel_cnst:
                relax_1 = cp.Variable()
                relax_2 = cp.Variable()
                obj = weight_speed_noise*cp.norm(v_recon - v_filtered, 2)**2 + cp.std(a_recon)**2 + cp.sum(lam_soft_cnst * (a_recon/a_driver_max - 1)) + relax_1**2 + relax_2**2
                cnst = [
                    v_recon >= 0,
                ...
                obj = weight_speed_noise*cp.norm(v_recon - v_filtered, 2)**2 + cp.std(a_recon)**2 + cp.sum(lam_soft_cnst * (a_recon/a_driver_max - 1))
                cnst = [
                    v_recon >= 0,
                    cp.sum(v_recon)/FILTERING_SAMPLING_FREQUENCY == x_noised[-1] - x_noised[0],
                    a_recon - a_min >= 0,
...
# #############################################################################
def _label_speed_acceleration_region(trajectory_df: pd.DataFrame, vehicle_info_df: pd.DataFrame, common_driving_style: float = 0.5) -> pd.DataFrame:    
    unique_vehicle_ids = trajectory_df["Vehicle_ID"].unique()
    mod_trajectory_df = None
    for vehicle_id in unique_vehicle_ids:
        vehicle_df = trajectory_df[trajectory_df["Vehicle_ID"] == vehicle_id].copy()
        vehicle_df["Region"] = pd.NA
        mfc_car_id = vehicle_info_df.loc[vehicle_info_df["Vehicle_ID"] == vehicle_id, "MFC_CarID"].item()
        with open(os.path.join(VEHICLE_INFO_PATH, f"ID{mfc_car_id}_AccelCapInterp.pkl"), 'rb') as f:
            accel_max_spl = pickle.load(f)
        with open(os.path.join(VEHICLE_INFO_PATH, f"ID{mfc_car_id}_DecelCapInterp.pkl"), 'rb') as f:
        ...
        for idx, row in vehicle_df.iterrows():
            ...
            if accel > a_max:
                vehicle_df.loc[idx, "Region"] = "A"
            elif accel >= common_driving_style*a_max:
                vehicle_df.loc[idx, "Region"] = "B"
            elif accel >= d_min:
                vehicle_df.loc[idx, "Region"] = "C"
            else:
                vehicle_df.loc[idx, "Region"] = "D"
...
def _vehicle_dynamics_constraint(trajectory_df: pd.DataFrame, vehicle_info_df: pd.DataFrame) -> pd.DataFrame:
    if "Region" not in trajectory_df.columns:
        trajectory_df = _label_speed_acceleration_region(trajectory_df, vehicle_info_df)
    ...
    for vehicle_id in unique_vehicle_ids:
        ...
        subdf = vehicle_df[(vehicle_df["Region"] == "A") | (vehicle_df["Region"] == "D")]
        modified_ratio = modified_ratio + len(subdf)
        for idx, row in subdf.iterrows():
            speed = row["v_Vel"]
            region = row["Region"]
            if region == "A":
                vehicle_df.loc[idx, "v_Accel"] = accel_max_spl(speed)
                vehicle_df.loc[idx, "Region"] = "B"
            elif region == "D":
                vehicle_df.loc[idx, "v_Accel"] = decel_min_spl(speed)
                vehicle_df.loc[idx, "Region"] = "C"
    ...
    modified_ratio = modified_ratio / len(mod_trajectory_df)    
    return mod_trajectory_df, modified_ratio


def _driver_dynamics_compliance(trajectory_df: pd.DataFrame, vehicle_info_df: pd.DataFrame, time_window: float = 1.5):
    if "Region" not in trajectory_df.columns:
        trajectory_df = _label_speed_acceleration_region(trajectory_df, vehicle_info_df)
...
def apply_physics_informed_butterworth_filter(trajectory_df: pd.DataFrame, vehicle_info_df: pd.DataFrame,
                                              init_cutoff_freq = 0.9, max_filter_strength = 0.05, cutoff_freq_step: float = 0.05,
                                              region_c_threshold: float = 0.95) -> pd.DataFrame:
    filtered_trajectory_df = _label_speed_acceleration_region(trajectory_df, vehicle_info_df)
    filtered_trajectory_df, modified_ratio = _vehicle_dynamics_constraint(filtered_trajectory_df, vehicle_info_df)
    region_c_ratio, median_local_accel_std = _driver_dynamics_compliance(filtered_trajectory_df, vehicle_info_df)
    ...
    for iter in range(20):
        ...
        filtered_trajectory_df = _label_speed_acceleration_region(filtered_trajectory_df, vehicle_info_df)
        filtered_trajectory_df, modified_ratio = _vehicle_dynamics_constraint(filtered_trajectory_df, vehicle_info_df)
        region_c_ratio, median_local_accel_std = _driver_dynamics_compliance(filtered_trajectory_df, vehicle_info_df)
