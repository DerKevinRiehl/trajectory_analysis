...
def _correctZeroDiffs(vals):
    ...
    for val in vals:
        if last_val is None:
            ...
            if abs(val - last_val) <= integration_correct_threshold:
...
def _correctZeroDiffsRepeatedly(vals):
    for i in range(0, integration_correct_repetitions):
...
vehicle_proceeding_order_file_path = "../data/5_vehicle_relationships/proceeding_order/DJI_0933.MOV.txt"
trajectory_type = "obb"
first_vehicle = "VEHICLE_1"
last_vehicle = "VEHICLE_14"
integration_correct_threshold = 0.00001
integration_correct_repetitions = 50
...
trajectory_df["space_headway"] = trajectory_df["x_lane_ref_x"] - trajectory_df["x_lane_ref_y"]
...
# for vehicle_id in unique_vehicles:
#     sel = trajectory_df[trajectory_df["vehicle"]==vehicle_id]
#     plt.plot(sel["frame_nr"], sel["headway"], label=vehicle_id)
# plt.legend()

# space_headways = []
# for idx, row for trajectory_df.iterrows():
