...
def setup_broken_axis(gs_cell, title, data, ylim_top, ylim_bottom, xlabels):
    ...
    plt.sca(ax_top)
    plt.title(title)
    data.plot.bar(ax=ax_top, edgecolor="black", color=["forestgreen","aquamarine","darkseagreen"], width=0.75, legend=False, zorder=3)
    ax_top.set_ylim(ylim_top)
    ax_top.spines['bottom'].set_visible(False)
    ax_top.tick_params(labelbottom=False, bottom=False)
    ax_top.grid(zorder=0)
    
    data.plot.bar(ax=ax_bottom, edgecolor="black", color=["forestgreen","aquamarine","darkseagreen"], width=0.75, legend=False, zorder=3)
...
v1 = np.amin([vehicle_df["v_Vel"].iloc[t1_idx:t2_idx], vehicle_df_CvxOpt["v_Vel"].iloc[t1_idx:t2_idx], vehicle_df_Wavelet["v_Vel"].iloc[t1_idx:t2_idx], vehicle_df_PIButterworth["v_Vel"].iloc[t1_idx:t2_idx]])-0.15
v2 = np.amax([vehicle_df["v_Vel"].iloc[t1_idx:t2_idx], vehicle_df_CvxOpt["v_Vel"].iloc[t1_idx:t2_idx], vehicle_df_Wavelet["v_Vel"].iloc[t1_idx:t2_idx], vehicle_df_PIButterworth["v_Vel"].iloc[t1_idx:t2_idx]])+0.15
axs[0][0].add_patch(
...
t1_idx, t2_idx = np.argwhere(time_arr == t1)[0][0], np.argwhere(time_arr == t2)[0][0]
d1 = np.amin([vehicle_df["Lane_X"].iloc[t1_idx], vehicle_df_CvxOpt["Lane_X"].iloc[t1_idx], vehicle_df_Wavelet["Lane_X"].iloc[t1_idx], vehicle_df_PIButterworth["Lane_X"].iloc[t1_idx]]) - vehicle_df["Lane_X"].iloc[0]
d2 = np.amax([vehicle_df["Lane_X"].iloc[t2_idx], vehicle_df_CvxOpt["Lane_X"].iloc[t2_idx], vehicle_df_Wavelet["Lane_X"].iloc[t2_idx], vehicle_df_PIButterworth["Lane_X"].iloc[t2_idx]]) - vehicle_df["Lane_X"].iloc[0]
axs[1][0].add_patch(
...
err_lanex_pibutter = abs(vehicle_df_PIButterworth["Lane_X"].to_numpy() - vehicle_df["Lane_X"].to_numpy())
...
axs[1][1].plot(vehicle_df_PIButterworth["Global_Time"], err_lanex_pibutter, label="PI-Butterworth Reconstruction", linestyle=":", alpha=0.75, color=color_pibutter)
...
df1 = pd.DataFrame(np.asarray([maxAbsErr_CvxOpt, maxAbsErr_PIButterworth, maxAbsErr_Wavelet]).transpose(), columns=["Proposed Reconstruction", "PI-Butterworth Reconstruction", "Wavelet-Based Reconstruction"])
ax1 = setup_broken_axis(gs[0], "Max. Absolute Position Error [m]", df1, (1, 20), (0, 1), xlabels=unique_vehicle_ids)
ax1.set_ylabel("")
df2 = pd.DataFrame(np.asarray([meanAbsErr_CvxOpt, meanAbsErr_PIButterworth, meanAbsErr_Wavelet]).transpose(), columns=["Proposed Reconstruction", "PI-Butterworth Reconstruction", "Wavelet-Based Reconstruction"])
...
axs[1, 0] = plot_oblique_trajectories(df_PIButterworth, start_frame=0, end_frame=None, vehicle_labels_in_plot=False, axis=axs[1, 0])
axs[1, 0].get_legend().remove()
axs[1, 0].set_xlabel("Time [s]")
axs[1, 0].set_ylabel("Oblique Lane-Coordinate Position")
axs[1, 0].set_title("(c) PI-Butterworth Reconstruction", fontweight="bold")
